...
import { buildAvatar } from "../shared/avatars.js";
...
function buildSwitchSign(id, anchor, disposables) {
  ...
  // arrowGeometry() points along -Z after its rotateX(-90), so add PI to face the target dir.
  const angleFor = (d) => Math.atan2(d.x, d.z) + Math.PI;
...
// ---------------------------------------------------------------------------
// Lobby: 3D conductor podiums (character-select), live avatar/name/ready per slot
// ---------------------------------------------------------------------------

function nameLabelSprite(text) {
  const cv = document.createElement("canvas");
  cv.width = 512;
  cv.height = 128;
  const ctx = cv.getContext("2d");
  paintNameLabel(ctx, cv, text);
  const tex = new THREE.CanvasTexture(cv);
  tex.anisotropy = 4;
...
function paintNameLabel(ctx, cv, text) {
  ctx.clearRect(0, 0, cv.width, cv.height);
  const r = 34;
  ctx.fillStyle = "rgba(12,22,38,0.62)";
  roundRectPath(ctx, 8, 26, cv.width - 16, 76, r);
  ctx.fill();
  ctx.lineWidth = 3;
  ctx.strokeStyle = "rgba(255,255,255,0.28)";
  ctx.stroke();
  ctx.font = "700 52px DynaPuff, Sniglet, sans-serif";
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillStyle = "#ffffff";
  ctx.fillText(text || "", cv.width / 2, 66, cv.width - 48);
}

export function buildLobbyPodiums(sc, count) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const slotColor = new THREE.Color(COLORS.slots[i] || "#cccccc");

    const podium = new THREE.Group();
    podium.position.set(x, 0, 0);

    const baseMat = new THREE.MeshStandardMaterial({ color: slotColor.clone().offsetHSL(0, -0.15, -0.08), roughness: 0.7, metalness: 0.1 });
    const base = new THREE.Mesh(new THREE.CylinderGeometry(1.55, 1.75, 0.55, 32), baseMat);
    base.position.y = 0.275;
    base.receiveShadow = true;
    base.castShadow = true;
    podium.add(base);
    const topMat = new THREE.MeshStandardMaterial({ color: slotColor, roughness: 0.55, metalness: 0.15, emissive: slotColor.clone().multiplyScalar(0.15) });
    const top = new THREE.Mesh(new THREE.CylinderGeometry(1.5, 1.55, 0.12, 32), topMat);
    top.position.y = 0.6;
    podium.add(top);

    const avatarHolder = new THREE.Group();
    avatarHolder.position.y = 0.66;
    podium.add(avatarHolder);

    const label = nameLabelSprite("");
    label.position.set(0, 4.4, 0);
    label.visible = false;
    podium.add(label);

    // ready ring (glowing) under avatar
    const ringMat = new THREE.MeshStandardMaterial({ color: 0x43c463, emissive: 0x2fa050, emissiveIntensity: 0.8, roughness: 0.4 });
    const ring = new THREE.Mesh(new THREE.TorusGeometry(1.35, 0.09, 12, 40), ringMat);
    ...
    group.add(podium);
    slots.push({ podium, avatarHolder, label, ring, baseMat, topMat, ringMat, avatarModel: null, avatarIndex: -1, present: false });
  }

  if (sc && sc.scene) sc.scene.add(group);

  function ensureAvatar(slot, idx) {
    if (slot.avatarIndex === idx && slot.avatarModel) return;
    if (slot.avatarModel) {
      slot.avatarHolder.remove(slot.avatarModel);
      slot.avatarModel.traverse((o) => {
        if (o.isMesh) { o.geometry && o.geometry.dispose(); const m = o.material; if (Array.isArray(m)) m.forEach((x) => x && x.dispose()); else m && m.dispose(); }
      });
    }
    const m = buildAvatar(idx, THREE);
    m.scale.setScalar(1.15);
    slot.avatarHolder.add(m);
    slot.avatarModel = m;
    slot.avatarIndex = idx;
  }

  const api = {
    group,
    count: n,
    setSlot(i, { present, avatar, name, ready }) {
      const slot = slots[i];
      if (!slot) return;
      slot.present = !!present;
      const idx = typeof avatar === "number" ? avatar : i;
      ensureAvatar(slot, idx);
      slot.avatarModel.visible = true;
      slot.avatarHolder.traverse((o) => { if (o.isMesh && o.material && o.material.opacity !== undefined) {} });
      const dim = !present;
      slot.podium.traverse((o) => { if (o.isMesh && o.material) { o.material.transparent = dim; o.material.opacity = dim ? 0.4 : 1; } });
      if (name) { paintNameLabel(slot.label._ctx, slot.label._cv, name); slot.label._tex.needsUpdate = true; slot.label.visible = true; }
      else slot.label.visible = false;
      slot.ring.visible = !!ready && !!present;
    },
    update(dt) {
      for (const s of slots) {
        if (s.avatarModel) s.avatarModel.rotation.y += dt * 0.7;
        if (s.ring.visible) { s.ring.position.y = 0.7 + Math.sin(performanceNow() * 0.004 + s.podium.position.x) * 0.06; }
      }
    },
    dispose() {
...
function performanceNow() {
  try { return performance.now(); } catch (e) { _pnStart += 16; return _pnStart; }
}
